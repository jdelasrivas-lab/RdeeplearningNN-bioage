...
deepNeuralNetwork.build <- function(x,y,
                                    ...
                                    drawDNN = FALSE, random.seed = 1, standarization = NULL){
...
deepNeuralNetwork.predict <- function(model.trained, data = X.test, standarization = NULL) {
  ...
  if(is.list(standarization)){
    new.data <- t(data)
    new.data <- as.matrix(plyr::ldply(lapply(X = standarization,function(x)deepNeuralNetwork.standarize(x = t(new.data),method = "genecomb",gene = as.character(x))), data.frame))
    new.data <- as.matrix(rbind(deepNeuralNetwork.standarize(data,method = "r"),new.data))
    new.data <- t(new.data)
  }else{
                                       ...
                                       standarization = NULL)
{
  ...
  if(is.list(standarization)){
    if(!require(plyr)){
      message("Package plyr not installed.")
      return(NULL)}
    X <- as.matrix(ldply(lapply(X = standarization,function(x)deepNeuralNetwork.standarize(x = t(X.raw.t),method = "genecomb",gene = as.character(x))), data.frame))
    X <- as.matrix(rbind(deepNeuralNetwork.standarize(t(X.raw.t),method = "r"),X))
    X <- t(X)
